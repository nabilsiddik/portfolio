...
import { BackendIcon, BootstrapIcon, CssIcon, FrontEndIcon, HtmlIcon, NextJsIcon, OtherSkillsIcon, ReactIcon, SassIcon, TailwindIcon } from '@/svg/SkillIcons';
      ...
      <div className="container mx-auto max-w-6xl px-10">
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              >
                ...
                <span className='font-bold tracking-wide text-center text-xl lg:text-2xl mt-3'>
                  {tab.name}
                </span>
              </div>
        ...
        <AnimatePresence mode='wait'>
          ...
          >
            ...
            <h3 className='text-2xl font-bold mb-7 text-gray-800 dark:text-white'>
              {tabs.find((t) => t.id === activeTab)?.name}
            </h3>

            <div className='prose dark:prose-invert grid grid-cols-5 gap-4'>
              {tabs.map((tab) => {
                return tab.content.map((contentItem) => {
                  return <Badge>
                    <div className='flex items-center gap-2 py-1 px-2'>
                      <span>{contentItem.icon}</span>
                      <span className='font-bold text-lg'>{contentItem.name}</span>
                    </div>
                  </Badge>
                })
              })}
